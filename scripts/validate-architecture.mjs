...
const inspectTsx = (file, source) => {
  ...
  const visit = (node) => {
    ...
    if (ts.isReturnStatement(node) && node.expression && isJsxExpression(node.expression)) {
      jsxReturns += 1;
    }
  ...
  if (componentDeclarations !== 1) {
    errors.push(`${path.relative(root, file)} must contain exactly one React component; found ${componentDeclarations}.`);
  }

  if (jsxReturns > 1) {
    errors.push(`${path.relative(root, file)} must contain at most one component-level JSX return; found ${jsxReturns}.`);
  }
...
const inspectComponentFolders = async () => {
  ...
  for (const category of categories) {
    ...
    for (const entry of entries) {
      if (!entry.isDirectory()) {
        errors.push(`${path.relative(root, categoryPath)} may contain only component folders.`);
        continue;
      }
      ...
      if (implementations.length !== 1) {
        errors.push(`${path.relative(root, folder)} must contain exactly one production TSX component.`);
      }
      if (tests.length !== 1) {
        errors.push(`${path.relative(root, folder)} must contain exactly one colocated component test.`);
      }
      if (!files.includes("index.ts")) {
        errors.push(`${path.relative(root, folder)} must contain index.ts.`);
      } else {
        const indexSource = await readFile(path.join(folder, "index.ts"), "utf8");
        if (!/export\s*\{\s*default\s*\}\s*from/.test(indexSource)) {
          errors.push(`${path.relative(root, folder)}/index.ts must expose a default export.`);
        }
      }
    }
  }
...
for (const file of projectFiles) {
  ...
  if (source.includes("\u2014")) {
    errors.push(`${path.relative(root, file)} contains a prohibited em dash character.`);
  }
}
...
console.log(`Architecture is valid across ${productionTsx.length} production TSX files.`);
