...
await Promise.all([
  writeFile(path.join(publicDirectory, "brand", "pr-monogram.svg"), monogramSvg),
...
const imageJobs = [
  {
    ...
    transform: (image) => image.resize(1600, 900, { fit: "cover", position: "top" }),
  },
  {
    input: "woku.png",
    output: "woku-evidence.webp",
    transform: (image) => image.extract({ left: 20, top: 18, width: 540, height: 460 }),
  },
  {
    input: "inpla.png",
  ...
  },
  {
    ...
    transform: (image) => image.resize({ width: 1080, withoutEnlargement: true }),
  },
  {
    input: "fracaso.png",
    output: "startup-closure-feature.webp",
    transform: (image) => image.extract({ left: 0, top: 0, width: 1103, height: 1240 }),
  },
  {
  ...
  },
  {
    ...
    transform: (image) => image.resize({ width: 549, withoutEnlargement: true }),
  },
];

for (const job of imageJobs) {
  const image = sharp(path.join(sourceDirectory, job.input)).flatten({ background: "#ffffff" });
  await job
    .transform(image)
...
pdf.setKeywords(["product leadership", "product design", "frontend development"]);
...
page.drawText("Concepción, Biobío, Chile  |  paula.riq.esco@gmail.com  |  linkedin.com/in/pauriquelme", {
  x: margin,
  y: 727,
  size: 8.3,
  font: regularFont,
...
y = drawWrapped(page, "Product: Product strategy, product discovery, customer research, roadmaps, UX/UI, prototyping, go-to-market, and agile delivery.", { y, size: 8.6, lineHeight: 11.3 });
y = drawWrapped(page, "Design: Figma, Adobe Illustrator, and Adobe Premiere Pro at advanced proficiency.", { y: y - 2, size: 8.6, lineHeight: 11.3 });
...
y = drawRole(page, "Co-founder & Brand Artisan, Product Design", "Inpla", "May 2025 - January 2026", "Chile", [
...
y = drawRole(page, "CEO & Co-founder", "stow SpA", "October 2020 - December 2022", "Concepción, Chile", [
...
page = createPage();
page.drawText("PAULA RIQUELME", { x: margin, y: 770, size: 18, font: boldFont });
...
y = drawRole(page, "Maintenance Planning Engineer", "Essbio", "May 2019 - July 2021", "Concepción, Chile", [
...
y = drawRole(page, "Lecturer", "Universidad de Concepción", "2023", "Chile", [
  "Taught Business Management and coached students in Project Formulation and Evaluation.",
], y);
y = drawRole(page, "CEO", "Orvita", "2018 - 2019", "Chile", [
...
y = drawWrapped(page, "Master's in Innovation and Technology Entrepreneurship | Universidad de Concepción, 2023", { y, size: 9.2, font: boldFont, lineHeight: 12 });
y = drawWrapped(page, "Industrial Engineering | Universidad de Concepción, 2018", { y: y - 3, size: 9.2, font: boldFont, lineHeight: 12 }) - 10;
y = drawSectionTitle(page, "Recognition", y);
y = drawWrapped(page, "Best Undergraduate Paper | OPTIMA 2017", { y, size: 10.5, font: boldFont, lineHeight: 13 });
